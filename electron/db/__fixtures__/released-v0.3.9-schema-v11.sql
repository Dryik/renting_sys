-- FROZEN FIXTURE — released schema from tag v0.3.9 (schema_version 11).
...
CREATE TABLE rental_collateral_items (
      id integer primary key autoincrement,
      rental_id integer not null references rentals(id),
...
CREATE TABLE rentals (
      ...
      created_at text not null,
      updated_at text not null
    , sales_user_id integer references users(id), commission_rate_per_day real not null default 0, commission_amount real not null default 0);
...
CREATE INDEX accessories_is_active_idx on accessories(is_active);
...
CREATE INDEX employee_loan_payments_loan_id_idx on employee_loan_payments(loan_id);

CREATE INDEX employee_loan_payments_payment_date_idx on employee_loan_payments(payment_date);

CREATE INDEX employee_loan_payments_status_idx on employee_loan_payments(status);

CREATE INDEX employee_loans_employee_user_id_idx on employee_loans(employee_user_id);

CREATE INDEX employee_loans_issued_at_idx on employee_loans(issued_at);

CREATE UNIQUE INDEX employee_loans_loan_no_idx on employee_loans(loan_no);
...
CREATE INDEX rental_accessories_accessory_id_idx on rental_accessories(accessory_id);

CREATE INDEX rental_accessories_rental_id_idx on rental_accessories(rental_id);

CREATE INDEX rental_collateral_items_rental_id_idx on rental_collateral_items(rental_id);

CREATE INDEX rental_collateral_items_status_idx on rental_collateral_items(status);
...
insert into app_settings (key, value) values ('schema_version', '11');
